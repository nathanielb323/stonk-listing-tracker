...
- derived narrative/theme column from company descriptions
- adjustable cutoff controls
- near-cutoff rows
- lighter product-style UI
"""
...
THEME_RULES = [
    ...
    ("Broker / Fintech", ["broker", "brokerage", "trading platform", "payments", "consumer finance", "banking", "lending", "financial technology"]),
...
def build_display_df(base_df, scores_db, min_vol, min_score, near_cutoff_count, exclude_current, selected_themes):
    if base_df.empty:
        return base_df
    ...
    near_cutoff = working[
        (
            ((working["Today Vol ($M)"] < min_vol) & (working["Today Vol ($M)"] >= max(0, min_vol - 150))) |
            ((working["Auto Score"] < min_score) & (working["Auto Score"] >= max(0, min_score - 1.0)))
        )
    ].copy()
    ...
    order_cols = [
        ...
        "Full R/R", "Recommendation", "Notes", "Business Summary",
    ]
    df = df[order_cols]
...
with st.sidebar:
    ...
    near_cutoff_count = st.slider("Near-cutoff extras", 0, 20, 10, step=1)
...
<div class="panel">
  <h3>Tracker</h3>
  <p>Theme is auto-derived from company descriptions and sector metadata. Manual inputs remain editable for Narrative, Not on HL, Momentum, MM Feasibility, and Notes.</p>
</div>
""",
    unsafe_allow_html=True,
)
